...
local function open_backpack(itemstack, user)
	...
	if ctrl.sneak then
		...
		local detached_inv = minetest.get_inventory({type="detached", name="is_backpacks_" .. name})
		...
		if detached_inv then
			...
			local formspec = "size[8,9.5]" ..
				"label[0,0;Industrial Survival Rucksack]" ..
				"list[detached:is_backpacks_" .. name .. ";main;0,0.5;8,4;]" ..
				"label[0,4.75;Inventar]" ..
				"list[current_player;main;0,5.3;8,1;]" ..
				"list[current_player;main;0,6.5;8,3;8]" ..
				"listring[detached:is_backpacks_" .. name .. ";main]" ..
				"listring[current_player;main]"

			minetest.show_formspec(name, "is_backpacks:backpack_fs", formspec)
			return itemstack
		end
	end
	return nil
...
for _, color_data in ipairs(colors) do
	...
	minetest.register_craftitem("is_backpacks:backpack_" .. color_name, {
		description = "Industrial Survival Rucksack (" .. uppercase_name .. ")",
		...
		inventory_image = "is_backpacks_base.png^[multiply:" .. hex_code .. "^is_backpacks_overlay.png",
