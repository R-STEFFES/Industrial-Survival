-- is_backpacks: init.lua
is_backpacks = {} -- Globaler Namespace
is_backpacks.slots = 32
...
local function save_backpack_data(inv, player)
	...
	if sheet:get_name():find("^is_backpacks:backpack_") then
...
minetest.register_on_joinplayer(function(player)
	...
	local inv = minetest.create_detached_inventory("is_backpacks_" .. name, {
		-- Verhindert das Verschieben von Rucksäcken INNERHALB des Rucksack-Inventars
		allow_move = function(inv, from_list, from_index, to_list, to_index, count, player)
			local stack = inv:get_stack(from_list, from_index)
			if stack:get_name():find("^is_backpacks:backpack_") then
				return 0
			end
			return count
		end,
	...
	inv:set_size("main", is_backpacks.slots)
end)

-- Weitere Dateien laden
local modpath = minetest.get_modpath("is_backpacks")
