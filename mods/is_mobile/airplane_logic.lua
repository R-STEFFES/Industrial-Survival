...
minetest.register_entity("is_mobile:airplane", {
    physical = true,
    collisionbox = {-1.5, -0.5, -1.5, 1.5, 1.5, 1.5},
    visual = "mesh",
    mesh = "Platzhalter_flieger.obj",
    textures = {"is_mobile_flieger.png"},
    stepheight = 0.6,
    ...
    on_activate = function(self, staticdata, dtime_s)
        ...
        local inv_name = "is_mobile:airplane_" .. self.id
    ...
    get_staticdata = function(self)
        local cargo_items = {}
        local fuel_items = {}
        local inv = minetest.get_inventory({type = "detached", name = "is_mobile:airplane_" .. (self.id or "")})
    ...
    on_step = function(self, dtime)
        ...
        if ctrl.aux1 then
            if not self.aux1_pressed then
                ...
                minetest.show_formspec(self.driver, "is_mobile:airplane_inv", get_airplane_formspec(self.id, self.fuel, self.max_fuel))
            end
        else
        ...
        if self.fuel < self.max_fuel then
            local inv = minetest.get_inventory({type = "detached", name = "is_mobile:airplane_" .. self.id})
            if inv then
