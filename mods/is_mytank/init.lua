...
function tank.check_and_calculate_tank(controller_pos)
    ...
    local function is_tank_part(name)
        return name == "is_mytank:wall" or
               name == "is_mytank:glass" or
               name == "is_mytank:inlet" or
               name == "is_mytank:outlet" or
               name == "is_mytank:controller" or
               name == "is_mytank:controller_active"
    end
        ...
        for y = min_p.y, max_p.y do
            for z = min_p.z, max_p.z do
                ...
                if is_edge_x and is_edge_y and is_edge_z then
                    ...
                    if node.name ~= "is_mytank:wall" then
    ...
    if valid_structure and #air_blocks > 0 then
        ...
        if current_node.name == "is_mytank:controller" then
...
function tank.set_tank_invalid(pos, current_node)
    ...
    if current_node.name == "is_mytank:controller_active" then
        minetest.swap_node(pos, {
            name = "is_mytank:controller",
            param1 = current_node.param1,
            param2 = current_node.param2
        })
    end
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    ...
    local x, y, z =
        formname:match(
            "^is_mytank:controller_([%-%d]+)_([%-%d]+)_([%-%d]+)$"
        )
...
minetest.register_node("is_mytank:controller", {
...
minetest.register_node("is_mytank:controller_active", {
...
minetest.register_node("is_mytank:wall", {
    description = "Tank Wand",
    tiles = {"mytank_wall.png"},
    groups = {cracky = 3}
})

minetest.register_node("is_mytank:glass", {
...
minetest.register_node("is_mytank:inlet", {
    description = "Tank Einlass",
    tiles = {"mytank_inlet.png"},
    groups = {cracky = 3}
})

minetest.register_node("is_mytank:outlet", {
...
minetest.register_entity("is_mytank:fluid_display", {
...
function tank.update_visuals(controller_pos, air_blocks)
    ...
    or #air_blocks == 0 then
        ...
        for _, obj in ipairs(objects) do
            ...
            if ent
            and ent.name == "is_mytank:fluid_display"
