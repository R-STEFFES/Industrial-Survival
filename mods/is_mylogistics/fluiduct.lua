...
local function find_tank_controller(pos)
    return minetest.find_node_near(pos, 10, {"is_mytank:controller_active", "is_mytank:controller"})
end
...
local function process_fluid_transport(pos)
    ...
    if amount < 200 then
        for _, dir in ipairs(directions) do
            ...
            if neighbor_node.name == "is_machines:steam_pump" then
            ...
            elseif neighbor_node.name == "is_mytank:outlet" then
    ...
    if amount > 0 and fluid ~= "" then
        ...
        for _, dir in ipairs(directions) do
            ...
            if nnode.name == "is_mytank:inlet" then
            ...
            -- GENERISCHE ERKENNUNG: Für alle Industrial Survival-Maschinen mit Fluid-Interface
            elseif minetest.get_item_group(nnode.name, "machine_fluid") > 0 then
                local nmeta = minetest.get_meta(npos)
                ...
                -- Prüfen, ob die getroffene Seite auf "Fluid In" (3) steht & Flüssigkeit Wasser ist
                if nmeta:get_int("side_" .. side) == 3 and fluid == "water" then
                    if nmeta:get_int("tank_amount") < 8000 then
                        table.insert(targets, {pos = npos, type = "machine"})
                    end
                end
            end
        end
        ...
        if #targets > 0 then
            ...
            if share < 1 then share = amount end
            ...
            for _, target in ipairs(targets) do
                ...
                -- Bucht das Wasser direkt in den Maschinentank (Wascher & Abfüller)
                elseif target.type == "machine" then
                    ...
                    amount = amount - t_final
                else -- Originales Verhalten für "duct"
                    local nmeta = minetest.get_meta(target.pos)
...
local function register_duct(name, desc, tiles, alpha)
    ...
    for _, v in ipairs(variants) do
        ...
        minetest.register_node(name .. v.suffix, {
            ...
            connects_to = {"group:fluiduct", "is_mytank:inlet", "is_mytank:outlet", "group:machine_fluid"},
...
register_duct("is_mylogistics:fluiduct", "Fluiduct (Standard)", {"mylogistics_fluiduct.png"}, nil)
register_duct("is_mylogistics:fluiduct_glass", "Fluiduct (Glas)", {"mylogistics_fluiduct_glass.png"}, "blend")
