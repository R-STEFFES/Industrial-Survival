...
for _, elem in ipairs(elements) do
    ...
    for _, t in ipairs(tools) do
        ...
        local tool_texture = "is_core_tool_head_" .. t[1] .. ".png^[multiply:" .. elem.color .. "^is_core_" .. t[1] .. "_handle.png"

        minetest.register_tool("is_core:" .. t[1] .. "_" .. elem.name, {
    ...
    for _, arm in ipairs(armor_types) do
        local armor_name = "is_core:" .. arm.suffix .. "_" .. elem.name

        -- Das Inventar-Icon
        local inv_image = "is_core_inv_" .. arm.suffix .. ".png^[multiply:" .. elem.color

        -- Der 3d_armor Konsolen-Fehler-Fix
        local dynamic_texture = "is_core_armor_" .. arm.suffix .. ".png^[multiply:" .. elem.color .. "^[combine:1x1:0,0=blank"
        local dynamic_preview = "is_core_preview_" .. arm.suffix .. ".png^[multiply:" .. elem.color .. "^[combine:1x1:0,0=blank"
    ...
    -- 3. CRAFTING REZEPTE
    ---------------------------------------------------------------------------
    local ingot = "is_core:ingot_" .. elem.name
    local stick = "default:stick"

    -- Werkzeuge
    minetest.register_craft({
        output = "is_core:pick_" .. elem.name,
        recipe = {
            {ingot, ingot, ingot},
            {"",    stick, ""},
            {"",    stick, ""}
    ...
    minetest.register_craft({
        output = "is_core:shovel_" .. elem.name,
        recipe = {
            {ingot, "", ""},
            {stick, "", ""},
            {stick, "", ""}
    ...
    minetest.register_craft({
        output = "is_core:sword_" .. elem.name,
        recipe = {
            {ingot, "", ""},
            {ingot, "", ""},
            {stick, "", ""}
    ...
    -- Rüstungen
    minetest.register_craft({
        output = "is_core:helmet_" .. elem.name,
        recipe = {
            {ingot, ingot, ingot},
            {ingot, "",    ingot},
            {"",    "",    ""}
    ...
    minetest.register_craft({
        output = "is_core:chestplate_" .. elem.name,
        recipe = {
            {ingot, "",    ingot},
            {ingot, ingot, ingot},
            {ingot, ingot, ingot}
    ...
    minetest.register_craft({
        output = "is_core:leggings_" .. elem.name,
        recipe = {
            {ingot, ingot, ingot},
            {ingot, "",    ingot},
            {ingot, "",    ingot}
    ...
    minetest.register_craft({
        output = "is_core:boots_" .. elem.name,
        recipe = {
            {ingot, "",    ingot},
            {ingot, "",    ingot},
            {"",    "",    ""}
