...
is_machines.quarry_entities = is_machines.quarry_entities or {}
...
local function clear_quarry_frames(pos, meta)
    ...
    if queue_str and queue_str ~= "" then
        ...
        if frame_queue then
            for _, fpos in ipairs(frame_queue) do
                if minetest.get_node(fpos).name == "is_machines:quarry_frame" then
    ...
    if is_machines.quarry_entities[pos_hash] then
        local refs = is_machines.quarry_entities[pos_hash]
        ...
        is_machines.quarry_entities[pos_hash] = nil
    end
    ...
    for _, obj in ipairs(minetest.get_objects_inside_radius(pos, search_radius)) do
        ...
        if ent and ent.quarry_pos and vector.equals(ent.quarry_pos, pos) then
            if ent.name == "is_machines:gantry_x" or
               ent.name == "is_machines:gantry_carrier" or
               ent.name == "is_machines:drill_pipe" then
                obj:remove()
            end
        end
    end
...
function is_machines.update_quarry_formspec(pos)
    ...
    if inv then
        for i = 1, 4 do
            ...
            if stack:get_name() == "is_machines:upgrade_range" then
...
local quarry_def = {
    ...
    on_construct = function(pos)
        ...
        is_machines.update_quarry_formspec(pos)
    end,
    ...
    allow_metadata_inventory_put = function(pos, listname, index, stack, player)
        if listname == "upgrades" then
            ...
            if stack:get_name() == "is_machines:upgrade_range" or stack:get_name():find("is_machines:upgrade_speed_") then
    ...
    allow_metadata_inventory_move = function(pos, from_list, from_index, to_list, to_index, count, player)
        ...
        if status ~= "idle" and status ~= "completed" then
            ...
            if from_list == "upgrades" or to_list == "upgrades" then
                ...
                if stack:get_name() == "is_machines:upgrade_range" then return 0 end
            end
        end
        return count
    end,
    ...
    on_timer = function(pos, elapsed)
        ...
        if status ~= "paused" then
            ...
            if status == "building" or status == "clearing" or status == "digging" then
                ...
                for i = 1, 4 do
                    ...
                    if name:find("is_machines:upgrade_speed_") then
                        local lvl = tonumber(name:match("is_machines:upgrade_speed_(%d+)"))
                        if lvl then
                ...
                for op = 1, operations do
                    ...
                    if status == "building" then
                        ...
                        if frame_queue and idx <= #frame_queue then
                            ...
                            if current_node.name ~= "air" and current_node.name ~= "ignore" and current_node.name ~= "is_machines:quarry_frame" then
                                if not minetest.is_protected(fpos, "") then
                                    ...
                                    if fits then
                                        ...
                                        minetest.set_node(fpos, {name = "is_machines:quarry_frame"})
                    ...
                    elseif status == "clearing" then
                        ...
                        while scanned < 30 and not cleared_something and status == "clearing" do
                            ...
                            if cnode.name ~= "air" and cnode.name ~= "ignore" and cnode.name ~= "is_machines:quarry_frame" then
        ...
        if status == "clearing" or status == "clearing_full" or status == "digging" or status == "full" or status == "paused" then
            local pos_hash = minetest.hash_node_position(pos)
            is_machines.quarry_entities[pos_hash] = is_machines.quarry_entities[pos_hash] or {}
            local refs = is_machines.quarry_entities[pos_hash]
            ...
            if not gantry_x_obj or not carrier_obj or not pipe_obj then
                ...
                for _, obj in ipairs(minetest.get_objects_inside_radius(pos, search_radius)) do
                    local ent = obj:get_luaentity()
                    if ent and ent.quarry_pos and vector.equals(ent.quarry_pos, pos) then
                        if ent.name == "is_machines:gantry_x" and not gantry_x_obj then
                            gantry_x_obj = obj; refs.gantry_x = obj
                        elseif ent.name == "is_machines:gantry_carrier" and not carrier_obj then
                            carrier_obj = obj; refs.carrier = obj
                        elseif ent.name == "is_machines:drill_pipe" and not pipe_obj then
            ...
            if not gantry_x_obj then
                gantry_x_obj = minetest.add_entity({x=pos.x, y=pos.y+3, z=pos.z}, "is_machines:gantry_x")
            ...
            if not carrier_obj then
                carrier_obj = minetest.add_entity({x=pos.x, y=pos.y+3, z=pos.z}, "is_machines:gantry_carrier")
                if carrier_obj then
            ...
            if not pipe_obj then
                pipe_obj = minetest.add_entity({x=pos.x, y=pos.y+3, z=pos.z}, "is_machines:drill_pipe")
                if pipe_obj then
        ...
        local active_node_name = "is_machines:quarry_active"
        local inactive_node_name = "is_machines:quarry"
...
minetest.register_entity("is_machines:gantry_x", gantry_x_def)
...
minetest.register_entity("is_machines:gantry_carrier", gantry_carrier_def)
...
minetest.register_entity("is_machines:drill_pipe", drill_pipe_def)

-- =======================================================================
-- REGISTRIERUNG RAHMEN-NODE & QUARRY NODES
-- =======================================================================

minetest.register_node("is_machines:quarry_frame", {
...
minetest.register_node("is_machines:quarry", q_inactive)
...
minetest.register_node("is_machines:quarry_active", q_active)
...
for i = 1, 10 do
    minetest.register_craftitem("is_machines:upgrade_speed_" .. i, {
...
minetest.register_craftitem("is_machines:upgrade_range", {
