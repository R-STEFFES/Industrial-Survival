...
function is_machines.update_elecfurnace_formspec(pos)
...
local furnace_def = {
    ...
    on_construct = function(pos)
        ...
        is_machines.update_elecfurnace_formspec(pos)
    end,
    ...
    on_timer = function(pos, elapsed)
        ...
        if is_smelting then
            if node.name ~= "is_machines:electric_furnace_active" then
                minetest.swap_node(pos, {name = "is_machines:electric_furnace_active", param2 = node.param2})
            end
        else
            if node.name ~= "is_machines:electric_furnace" then
                minetest.swap_node(pos, {name = "is_machines:electric_furnace", param2 = node.param2})
            end
        end
...
minetest.register_node("is_machines:electric_furnace", furn_inactive)
...
minetest.register_node("is_machines:electric_furnace_active", furn_active)
