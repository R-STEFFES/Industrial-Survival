...
function is_machines.update_generator_formspec(pos)
...
local generator_def = {
    ...
    on_construct = function(pos)
        ...
        is_machines.update_generator_formspec(pos)
    end,
    ...
    on_timer = function(pos, elapsed)
        ...
        if burning or burn_time > 0 then
            ...
            if node.name ~= "is_machines:generator_active" then
                minetest.swap_node(pos, {name = "is_machines:generator_active", param2 = node.param2})
            end
        else
            meta:set_string("infotext", "Generator: Inaktiv\nEnergie: " .. energy .. " EU")
            if node.name ~= "is_machines:generator" then
                minetest.swap_node(pos, {name = "is_machines:generator", param2 = node.param2})
            end
        end
        ...
        if energy > 0 then
            if is_machines.push_energy_network then
                local drawn = is_machines.push_energy_network(pos, energy)
                if drawn > 0 then
                    energy = energy - drawn
                    meta:set_int("energy", energy)
                end
            else
                minetest.log("error", "[is_machines] push_energy_network fehlt in init.lua!")
            end
        end
...
minetest.register_node("is_machines:generator", def_inactive)
...
minetest.register_node("is_machines:generator_active", def_active)

function is_machines.draw_energy_from_node(pos, amount)
    local meta = minetest.get_meta(pos)
...
minetest.register_lbm({
    name = "is_machines:rekindle_generators",
    nodenames = {"is_machines:generator", "is_machines:generator_active"},
