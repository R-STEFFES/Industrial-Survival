-- is_terrain/mapgen_custom.lua
-- Benutzerdefinierter Terrain-Generator für den Singlenode-Mapgen.
-- Synchronisiert sich exakt mit den Biomen aus biomes.lua (is_terrain)
...
minetest.log("action", "[is_terrain] Singlenode erkannt – Custom Terrain Generator wird aktiviert.")
...
local c_stone         = get_node_id("is_core:stone")
local c_basalt        = get_node_id("is_core:basalt")
local c_granite       = get_node_id("is_core:granite")

local crystal_ids = {}

minetest.register_on_mods_loaded(function()
    if is_core and is_core.crystals then
        for _, nodename in ipairs(is_core.crystals) do
    ...
    if #crystal_ids == 0 then
        local fallbacks = {"is_core:crystal_red", "is_core:crystal_blue", "is_core:crystal_green"}
...
local function cache_biome(name, b_def)
    table.insert(cached_biomes, {
        ...
        stone = get_node_id(b_def.node_stone, "is_core:stone"),
...
if is_terrain and is_terrain.biome_list then
    for _, name in ipairs(is_terrain.biome_list) do
...
minetest.log("action", "[is_terrain] " .. #cached_biomes .. " is_terrain-Biome erfolgreich in Custom Mapgen geladen.")
...
minetest.log("action", "[is_terrain] Custom Terrain Generator (Minecraft Caves & Nether Logic) erfolgreich aktualisiert!")
