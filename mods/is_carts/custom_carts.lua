...
motor_cart.initial_properties.textures = {"is_carts_motor_cart.png"}
...
function motor_cart:on_rightclick(clicker)
	...
	if clicker:get_player_control().sneak then
		local inv_name = "is_carts_motor_" .. player_name
		...
		minetest.show_formspec(player_name, "is_carts:motor_fuel", formspec)
	else
			...
			if self.fuel_time and self.fuel_time > 0 then
				...
				if self.fuel_item and self.fuel_item ~= "" then
					...
					if fuel_res.time > 0 then
						...
						local inv = minetest.get_inventory({type="detached", name="is_carts_motor_" .. driver_name})
						if inv then
...
minetest.register_entity("is_carts:motor_cart", motor_cart)

-- Das tragbare Item zum Platzieren
minetest.register_craftitem("is_carts:motor_cart", {
	description = "Motor-Cart (Industrial Survival Edition)",
	inventory_image = "is_carts_motor_cart_item.png",
	on_place = function(itemstack, placer, pointed_thing)
		if pointed_thing.type ~= "node" then return end
		if carts:is_rail(pointed_thing.under) then
			minetest.add_entity(pointed_thing.under, "is_carts:motor_cart")
			itemstack:take_item()
		elseif carts:is_rail(pointed_thing.above) then
			minetest.add_entity(pointed_thing.above, "is_carts:motor_cart")
			itemstack:take_item()
		end
		return itemstack
	end,
