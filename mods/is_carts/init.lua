...
local S = minetest.get_translator("is_carts")
...
if base_cart then
	...
	function base_cart:get_staticdata()
		...
		data.is_cart_id = self.cart_id
		data.is_leader_id = self.leader_id
	...
	function base_cart:on_activate(staticdata, dtime_s)
		...
		if staticdata and staticdata ~= "" then
			...
			if data and type(data) == "table" then
				self.cart_id = data.is_cart_id
				self.leader_id = data.is_leader_id
			end
		end
...
minetest.register_craftitem("is_carts:coupling_chain", {
	description = "Koppelkette\nKlicke erst die Lok, dann Waggon 1, dann Waggon 2...",
	inventory_image = "is_carts_coupling_chain.png",
	stack_max = 1,
...
local modpath = minetest.get_modpath("is_carts")
